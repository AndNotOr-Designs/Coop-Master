...
// v2.00 moved most of the program out of the loop, cleaned up some unspecific names
...
// waterer monitoring
  ...
  float currentWatererLevel = 0;            // monitoring water leve in inches
...
// NOTE: to set clock: change rtc.write_protect to false, set time  in Time t(year, mon, day, hour, min, sec, day of week (sunday = 1))
  ...
  rtc.halt(false);
  //Time t(2019, 10, 27, 17, 53, 00, 1);       //initialize the time
...
void sendToESP32() {
  if ((darknessHasSet == true) && (ambientLightSensorReading > 40)) {
...
void sendToThingSpeak() {
  ...
  Serial.println("sensor - 2.5 sec delay - 1");
  delay(2500);
...
//  Serial.println("sensor - 2.5 sec delay - 2");
//  delay(2500);
  ...
  Serial.println("sensor - 2.5 sec delay - 3");
  delay(2500);
  if(Serial2.available()) {
    while(Serial2.available()) {
      char c = Serial2.read();
...
//  Serial.println("sensor - 2.5 sec delay - 4");
//  delay(2500);
  ...
  Serial.println("end sensor - 2.5 sec delay - 5");
  delay(2500);
  if(Serial2.available()) {
    while(Serial2.available()) {
      char c = Serial2.read();
...
/* currently only have 4 fields for 2nd channel  
  lightingStr +="&field5=";
  lightingStr +=String(barometricPressureInHG);
  lightingStr +="&field6=";
...
*/
  ...
  Serial.println("lighting - 2.5 sec delay - 1");
  delay(2500);
  if(Serial2.available()) {
    while(Serial2.available()) {
      char c = Serial2.read();
...
//  Serial.println("lighting - 2.5 sec delay - 2");
//  delay(2500);
  ...
  Serial.println("lighting - 2.5 sec delay - 3");
  delay(2500);
  if(Serial2.available()) {
    while(Serial2.available()) {
      char c = Serial2.read();
